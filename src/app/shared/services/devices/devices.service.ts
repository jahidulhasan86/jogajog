...
import { video_resulation } from '../../../global';
@Injectable({
	providedIn: 'root'
})
export class DevicesService {
	private readonly VIDEO_DEVICE = 'jagajagCallVideoDevice';
	private readonly AUDIO_DEVICE = 'jagajagCallAudioDevice';
	...
	async initDevices() {
		...
		if (this.hasVideoDeviceAvailable()) {
			...
			this.setVideoResulationType('480p');
			this.setVideoResulation(video_resulation['480p'].resulation);
			this.setVideoFrameRate(video_resulation['480p'].frameRate);
		}
	}
	...
	setVideoResulation(resulation: string) {
		this.videoResulation = resulation;
	}

	getVideoResulation(): string {
		return this.videoResulation;
	}
	setVideoResulationType(type: string) {
		this.videoResulationType = type;
	}

	getVideoResulationType(): string {
		return this.videoResulationType;
	}
