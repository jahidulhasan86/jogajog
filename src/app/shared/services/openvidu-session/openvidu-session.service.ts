...
export class OpenViduSessionService {
	...
	async connectWebcamSession(token: string, metaData): Promise<any> {
		if (!!token) {
			await this.webcamSession.connect(token, {
				clientData: this.getWebcamUserName(),
				// avatar: this.getWebCamAvatar(),
				avatar: metaData.profilePicUrl,
	...
	async connectScreenSession(token: string, metaData): Promise<any> {
		if (!!token) {
			await this.screenSession.connect(token, {
				clientData: this.getScreenUserName(),
				avatar: metaData.profilePicUrl,
				isTeacher: metaData.isTeacher,
				user_id: metaData.user_id,
				gender: metaData.gender,
	...
	changeResulation(videoSource, audioSource, resulation, frameRate): Promise<any> {
	...
	replaceTrack(videoSource: string, audioSource: string, mirror: boolean = true, resulation, frameRate): Promise<any> {
		return new Promise((resolve, reject) => {
			if (!!videoSource) {
				this.log.d('Replacing video track ' + videoSource);
				this.videoSource = videoSource;
	...
	async replaceScreenTrack(resulation, frameRate) {
		const videoSource = ScreenType.SCREEN;
		const hasAudio = !this.isWebCamEnabled();
		const properties = this.createProperties(videoSource, undefined, true, hasAudio, false, resulation, frameRate);
	...
	getMyOwnConnectionId() {
		return this.webcamUser?.getConnectionId();
	}

	getMyOwnScreenConnectionId() {
		return this.screenUser?.getConnectionId();
	}
	...
	setMeetingPassword(password: string) {
		this.meetingPassword = password;
	}

	getMeetingPassword(): string {
